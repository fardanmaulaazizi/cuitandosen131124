...
<div class="container-fluid" style="max-width: 1300px !important">
    <div class="container-fluid">
        <div class="d-flex justify-content-between">
            <h3>Sesi {{$value->tambahan->nama}} - {{ucwords($value->tambahan->tryout->nama)}}</h3>
        </div>
        <div class="row">
            <div class="col-md-9">
                <div class="card">
                    <div class="card-body">
                        @foreach ($value->tambahan->soal_sesis as $key => $so)
                        <div class="row mt-3 soal-soal {{$key != 0 ? 'd-none' : ''}}" id="soal-{{$key}}">
                            <div class="d-flex justify-content-between mb-5">
                                <h5><b>Soal {{$loop->iteration}}</b> <span class="span-kategori">{{ucwords($value->tambahan->nama)}}</span></h5>
                            ...
                            @foreach ($so->pilgan_sesis as $index => $pg)
                            @php
                ...
                <div class="card mt-2" style="position: sticky; top:250px">
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-center">
                            <a href="{{url('admin-tryout/hasil-test-sesi/'.$value->id)}}" class="btn btn-secondary ms-2">Selesai</a>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-12 d-flex justify-content-start">
                <div class="card w-100">
                    <div class="card-body">
                        ...
                        <hr>
                        @foreach ($value->tambahan->soal_sesis as $key2 => $so2)
...
<script>
    $(document).ready(function() {
        var subsession = {{$value->id}};
        var session = {{$value->session->id}};
        ...
        var maxDuration = {{ $value->tambahan->waktu }} * 60 * 1000;
        ...
        var countdownInterval = setInterval(function() {
            ...
            if (remainingTime <= 0) {
                ...
                $.ajax({
                    url: "/stop-session-sesi/"+subsession,
                    method:'post',
                    headers: {'X-CSRF-TOKEN' : "{{csrf_token()}}"},
                    success: function(response) {
                        console.log(response)
